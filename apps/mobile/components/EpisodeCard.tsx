...
export default function EpisodeCard({ episode, onPress, compact = false }: EpisodeCardProps) {
  ...
  return (
    ...
    >
      <Pressable onPress={onPress} style={{ padding: compact ? 12 : 16 }}>
        {/* Header */}
        <View
          style={{
            flexDirection: 'row',
        ...
        >
          <Text style={{ color: '#737373', fontSize: 10 }}>{episode.timeline.length} events</Text>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
            <Text style={{ color: '#737373', fontSize: 10 }}>{formatTime(episode.createdAt)}</Text>
