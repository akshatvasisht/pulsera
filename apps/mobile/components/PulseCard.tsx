...
export default function PulseCard({
  ...
  const barColor =
    clampedScore > 0.8 ? colors.critical : clampedScore > 0.5 ? colors.elevated : colors.safe

  const timeLabel = lastUpdated ? formatRelativeTime(lastUpdated) : 'No data'
    ...
    <GlassCard padding={16} borderRadius={16}>
      ...
      <View className="mt-3">
        <View className="flex-row items-center justify-between mb-1">
          <Text style={{ color: colors.textMuted, fontSize: 11 }}>Anomaly</Text>
      ...
      {/* Last updated */}
      <Text className="mt-2" style={{ color: colors.textMuted, fontSize: 10 }}>
        {timeLabel}
      </Text>
    </GlassCard>
  )
