...
import { type Episode, getPhaseLabel, getPhaseColor } from '@/lib/episodeSimulator'
...
const MEDICAL_GLOSSARY: { term: string; definition: string }[] = [
  {
    term: 'HRV',
    definition:
      'Heart Rate Variability — the variation in time between heartbeats. Lower HRV can indicate stress or a health event.',
  },
  {
    term: 'BPM',
    definition:
      'Beats Per Minute — how many times the heart beats in one minute. Resting adult range is typically 60–100 BPM.',
  },
  {
    term: 'Anomaly Score',
    definition:
      "A 0–100% severity rating computed from watch biometrics indicating how far vitals deviate from the wearer's baseline.",
  },
  {
    term: 'Fusion Analysis',
    definition:
      'Combines watch biometric data with visual check-in data to produce a more accurate severity assessment.',
  },
  {
    term: 'Sustained Elevated HR',
    definition:
      "Heart rate remains above the wearer's expected range for an extended period, which may signal distress.",
  },
  {
    term: 'Visual Check-In',
    definition:
      'A camera-based assessment that reads facial expression, breathing rate, and eye responsiveness to corroborate watch data.',
  },
]
...
export default function EpisodeDetailScreen() {
  ...
  try {
    ...
    if (parsed) {
      episode = {
        ...
        calmingStartedAt: parsed.calmingStartedAt ? new Date(parsed.calmingStartedAt) : undefined,
        calmingEndedAt: parsed.calmingEndedAt ? new Date(parsed.calmingEndedAt) : undefined,
  ...
  if (!episode) {
    return (
      ...
      >
        <Text style={{ color: '#a1a1a1', fontSize: 16 }}>Episode not found</Text>
      </View>
    )
  }
  ...
  return (
    ...
    >
      ...
      <GlassCard padding={16} borderRadius={glass.borderRadiusSmall} style={{ marginBottom: 16 }}>
        <View style={{ flexDirection: 'row', gap: 20 }}>
          ...
          <StatBox
            label="Type"
            value={
              ANOMALY_LABELS[episode.triggerData.anomalyType] ??
              episode.triggerData.anomalyType.replace(/_/g, ' ')
            }
            unit=""
            color="#1447e6"
          />
      ...
      {episode.presageData && (
        <>
          <SectionHeader title="Visual Check-In" icon="camera" color="#1447e6" />
          <GlassCard
            padding={16}
          ...
          >
            <View style={{ gap: 8 }}>
              ...
              <DataRow
                ...
                color={episode.presageData.facialExpression === 'calm' ? '#00bc7d' : '#fe9a00'}
              />
              <DataRow
                label="Eyes"
                value={episode.presageData.eyeResponsiveness}
                color={episode.presageData.eyeResponsiveness === 'normal' ? '#00bc7d' : '#fe9a00'}
              />
              <DataRow
                label="Confidence"
      ...
      {episode.fusionResult && (
        <>
          <SectionHeader title="Fusion Analysis" icon="git-merge" color="#ad46ff" />
          <GlassCard
            padding={16}
            borderRadius={glass.borderRadiusSmall}
            borderColor={
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  {episode.fusionResult.decision.replace(/_/g, ' ').toUpperCase()}
                </Text>
              </View>
              <Text
                style={{
            ...
            <View style={{ flexDirection: 'row', gap: 16, marginBottom: 12 }}>
              <ScoreBar label="Watch" score={episode.fusionResult.watchScore} color="#fe9a00" />
            ...
            <View style={{ alignItems: 'center', width: 20, marginRight: 12 }}>
              <View
                style={{
                  ...
                  height: 10,
                  borderRadius: 5,
                  backgroundColor: getPhaseColor(entry.phase as any),
                  marginTop: 2,
                }}
              />
      ...
      {isActive && (
        <View style={{ gap: 10, marginTop: 8 }}>
          ...
          >
            <Text style={{ color: '#ff6467', fontSize: 15, fontWeight: '700' }}>
...
function StatBox({
  ...
  return (
    <View style={{ flex: 1, alignItems: 'center' }}>
      ...
      <Text style={{ color, fontSize: 20, fontWeight: '800', marginTop: 2 }}>{value}</Text>
      {unit ? <Text style={{ color: '#737373', fontSize: 10 }}>{unit}</Text> : null}
    </View>
  )
}
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      >
        ...
        <Text style={{ color, fontSize: 11, fontWeight: '600' }}>{(score * 100).toFixed(0)}%</Text>
      </View>
      <View
        style={{
          height: 6,
  ...
  return (
    <>
      ...
      >
        ...
        <Ionicons name={expanded ? 'chevron-up' : 'chevron-down'} size={14} color="#a1a1a1" />
      </Pressable>
