...
export function simulateEpisodeProgression(episode: Episode): Episode {
  ...
  switch (episode.phase) {
    ...
    case 'fusing': {
      const fusion = runLocalFusion(episode.triggerData, episode.presageData)
      return {
        ...episode,
        ...
        resolution: fusion.decision === 'false_positive' ? 'false_positive' : undefined,
        resolvedAt: fusion.decision === 'false_positive' ? now : undefined,
        timeline: [
          ...episode.timeline,
          {
...
export function mapSmartSpectraToPresage(raw: {
  ...
  const confidence = (raw.pulseConfidence + raw.breathingConfidence) / 2
